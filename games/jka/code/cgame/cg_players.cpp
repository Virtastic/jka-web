...
//	const int iVariantCap = 2;	// test
	...
	{
		char *p = (char *)strchr(ppsTable[iEntryNum],'.');	// idTech3-web: C++ strchr const
...
static void _PlayerSplash( const vec3_t origin, const vec3_t velocity, const float radius, const int maxUp )
{
	...
	FX_AddOrientedParticle( -1, end, trace.plane.normal, NULL, NULL, 
								6.0f, radius + Q_flrandom() * 48.0f, 0, 
								alpha, 0.0f, 0.0f,
								WHITE, WHITE, 0.0f, 
								Q_flrandom() * 360, crandom() * 6.0f, NULL, NULL, 0.0f, 0 ,0, 1200, 
...
static void CG_PlayerSplash( centity_t *cent ) 
{
	...
	if ( cent->gent && cent->gent->client )
	{
		...
		if ( cent->gent->disconnectDebounceTime < cg.time ) // can't do these expanding ripples all the time
		{
			...
			cent->gent->disconnectDebounceTime = cg.time + 125 + Q_flrandom() * 50.0f;
		}
	}
}
...
/*	trace_t		trace;
	...
	{
		spark = qtrue;
		cent->gent->fx_time = cg.time + Q_flrandom() * 100 + 100;
	}
...
static void CG_ForceElectrocution( centity_t *cent, const vec3_t origin, vec3_t tempAngles, qhandle_t shader, qboolean alwaysDo = qfalse )
{
	...
	VectorMA( fxOrg, Q_flrandom() * 40 + 40, dir, fxOrg2 );

	trace_t	tr;

	CG_Trace( &tr, fxOrg, NULL, NULL, fxOrg2, -1, CONTENTS_SOLID );

	if ( tr.fraction < 1.0f || Q_flrandom() > 0.94f || alwaysDo )
	{
		FX_AddElectricity( -1, fxOrg, tr.endpos,
			1.5f, 4.0f, 0.0f, 
			1.0f, 0.5f, 0.0f,
			rgb, rgb, 0.0f,
			5.5f, Q_flrandom() * 50 + 100, shader, FX_ALPHA_LINEAR | FX_SIZE_LINEAR | FX_BRANCH | FX_GROW | FX_TAPER, -1, -1 );
	}
}
...
//	}
	...
	if ( gent->client->ps.powerups[PW_DISRUPTION] ) 
	{
		...
		if ( cg.time - ent->endTime < 1000 && (cg_timescale.value * cg_timescale.value * Q_flrandom()) > 0.05f )
		{
			vec3_t fxOrg;
			mdxaBone_t	boltMatrix;
...
static void CG_DoSaberLight( saberInfo_t *saber )
{
	...
	{//actually have something to do	
		...
		cgi_R_AddLightToScene( mid, diameter + (Q_flrandom()*8.0f), rgb[0], rgb[1], rgb[2] );
	}
}
...
static void CG_DoSaber( vec3_t origin, vec3_t dir, float length, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight )
{
	...
	{//FIXME: RGB combine all the colors of the sabers you're using into one averaged color!
		...
		cgi_R_AddLightToScene( mid, (length*1.4f) + (Q_flrandom()*3.0f), rgb[0], rgb[1], rgb[2] );
	}
...
//	byte			colors[4];
	...
	for ( i = 0, mf = markFragments ; i < numFragments ; i++, mf++ ) 
	{
		...
		for ( j = 0, v = verts ; j < mf->numPoints ; j++, v++ ) 
		{
			...
			v->st[0] = 0.5 + DotProduct( delta, axis[1] ) * (0.05f + Q_flrandom() * 0.03f);
			v->st[1] = 0.5 + DotProduct( delta, axis[2] ) * (0.15f + Q_flrandom() * 0.05f);
		}
		...
		mark->poly.numVerts = mf->numPoints;
		mark->color[0] = 215 + Q_flrandom() * 40.0f;
		mark->color[1] = 96 + Q_flrandom() * 32.0f;
		mark->color[2] = mark->color[3] = Q_flrandom()*15.0f;
		memcpy( mark->verts, verts, mf->numPoints * sizeof( verts[0] ) );
	}
}
...
//				VectorNormalize( cent->gent->client->renderInfo.muzzleDir );
	...
	if ( cent->currentState.number == 0 && cg.renderingThirdPerson )
	{
			...
			|| ( ps->weapon == WP_DEMP2 && ps->weaponstate == WEAPON_CHARGING_ALT ))
		{
			...
			FX_AddSprite( cent->gent->client->renderInfo.muzzlePoint, NULL, NULL, 3.0f * val * scale, 0.0f, 0.7f, 0.7f, WHITE, WHITE, Q_flrandom() * 360, 0.0f, 1.0f, shader, FX_USE_ALPHA );
		}
	}
}
