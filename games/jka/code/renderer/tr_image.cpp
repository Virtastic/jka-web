						  ...
						  USHORT *pUploadWidth, USHORT *pUploadHeight )
{
	if (format == GL_RGBA)
	{
		...
		// idTech3-web: the data below is always uploaded as GL_RGBA/GL_UNSIGNED_BYTE, and
		// WebGL1 requires internalformat == that pixel format. The GL 1.x component-count
		// shorthands (3/4), the sized formats (GL_RGB5/8, GL_RGBA4/8) and the S3TC
		// compressed enums selected above are all INVALID_VALUE here → the texture never
		// uploads and samples as solid black. With the world's lightmap modulate that
		// blacks out the entire scene. Same fix the RTCW/Wolf:ET renderer layer carries.
		*pformat = GL_RGBA;
#endif
...
#if defined(__EMSCRIPTEN__)
	// idTech3-web: desktop GL implicitly creates a texture object for any unused name
	// passed to glBindTexture, so the original just hands out 1024+N. WebGL texture
	// names are objects — an unknown name binds `undefined`, every upload silently
	// fails and the texture samples black (the whole scene renders black even though
	// the map loads fine). Allocate a real name instead. giTextureBindNum is still
	// incremented so anything keying off it keeps its original sequence.
...
static void R_CreateFogImage( void ) {
	...
	// idTech3-web: WebGL/GLES2 has no GL_TEXTURE_BORDER_COLOR (INVALID_OPERATION every boot).
	// The fog LUT's clamped edge is already the fully-fogged value (alpha 255 at s~1), so the
	// GL_CLAMP->GL_CLAMP_TO_EDGE mapping we already apply is visually identical to this white
	// border. The call is redundant under wasm; skip it.
...
#ifndef __EMSCRIPTEN__
	// idTech3-web: the dynamic-glow post-process uses GL_TEXTURE_RECTANGLE_EXT + GL_RGBA16 /
	// GL_FLOAT render targets, none of which WebGL1 supports — creating them here (which runs
	// unconditionally at image init) floods the console with INVALID_ENUM (rectangle target,
	// texImage2D/texParameter) every boot. Glow is force-disabled under wasm (see tr_init.cpp:
	// r_DynamicGlow is CVAR_ROM "0" and g_bDynamicGlowSupported stays false), so these targets
	// are never used; skip creating them.
