...
#if defined(__EMSCRIPTEN__)   // musl RAND_MAX is 2^31-1, not Win32's 0x7fff — mask to 15 bits
...
inline int Q_irand(int min, int max) {
	...
	// idTech3-web: these helpers assume Win32's 15-bit RAND_MAX (0x7fff). Emscripten's
	// musl libc has RAND_MAX == 2^31-1, so raw rand() overflows the `>>15` / /32768
	// scaling and returns wildly out-of-range values (Q_irand(0,6) -> 33274, indexing
	// arrays far out of bounds). Mask to 15 bits so the original arithmetic is exact.
...
inline float Q_flrandom() {
#if defined(__EMSCRIPTEN__)   // clamp musl's wider rand() to [0,1] like Win32's 15-bit RAND_MAX
