#!/usr/bin/env bash
# Build RTCW-MP (original GPL drop) to WebAssembly.
#
...
BUILD="$IDTECH3_ROOT/build-rtcw-mp"
mkdir -p "$BUILD" "$OUT"
...
DEFINES="-DMAC_STATIC= -DCPUSTRING=\"wasm32\" -DPATH_SEP='/' -DIDT3_CONST_URL -DIDT3_LOADDLL_FQPATH -DIDT3_FSROOT=\"/rtcwmp\""
...
compile() {  # compile <path-relative-to-SRC-or-abs> <tag>
  local src="$1" tag="$2"
  local o="$BUILD/${tag}.o"
  # Incremental: skip if the object is already newer than its source (set FORCE=1 to rebuild all).
