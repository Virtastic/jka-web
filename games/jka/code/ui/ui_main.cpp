...
// idTech3-web: min()/max() reached this TU as MSVC macros (windows.h), leaked in via
// /YX auto-PCH. Neither exists here without that, so make the TU self-contained --
// the same treatment the other Raven TUs got. NOT macros: jk_compat.h is force-
// included engine-wide and the macro form breaks files that use std::min/max.
// Every call site here is already same-type (int), so template deduction is clean.
#include <algorithm>
using std::min;
using std::max;
...
void Text_PaintWithCursor(float x, float y, float scale, vec4_t color, const char *text, int cursorPos, char cursor, int iMaxPixelWidth, int style, int iFontIndex) 
{
	...
	//
	char sTemp[1024];
	int iCopyCount = min((int)strlen(text), (int)cursorPos);
		iCopyCount = min((int)iCopyCount,(int)sizeof(sTemp));
