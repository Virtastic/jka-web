...
void GL_TexEnv( int env )
{
	...
	// idTech3-web: GL_TEXTURE_ENV_MODE must go through qglTexEnvi, not qglTexEnvf. Under
	// emscripten LEGACY_GL_EMULATION only the *integer* hook handles GL_TEXTURE_ENV_MODE;
	// the float hook silently drops it, so REPLACE/DECAL/ADD stages fell back to the default
	// MODULATE (additive glows dimmed, replace stages tinted by vertex colour). Identical on
	// desktop GL (the mode is an enum), a real fix under wasm.
	switch ( env )
	{
	case GL_MODULATE:
...
#ifdef _XBOX
	case GL_NONE:
		qglTexEnvi( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_NONE );
		break;
#endif
	default:
