...
#include "g_nav.h"
#include "../Ravl/CVec.h"	// idTech3-web: CVec3
...
void	Boba_DustFallNear(const vec3_t origin, int dustcount)
{
	...
	{
		testDirection[0] = (Q_flrandom() * 2.0f) - 1.0f;
		testDirection[1] = (Q_flrandom() * 2.0f) - 1.0f;
 			...
 			if (TIMER_Done(NPC, "SpookPlayerTimer"))
			{
				...
				case 1:
					Boba_Printf("SPOOK: Footsteps");
  					testDirection[0] =  (Q_flrandom() * 0.5f) - 1.0f;
	 		 		testDirection[0] += (testDirection[0]>0.0f)?(0.5f):(-0.5f);
					testDirection[1] = (Q_flrandom() * 0.5f) - 1.0f;
