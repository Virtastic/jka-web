...
// idTech3-web: wasm joins the Xbox 16-bit-index path. WebGL1 client-array draws
// accept ONLY GL_UNSIGNED_SHORT — emscripten's glDrawElements asserts exactly that
// when no element array buffer is bound (`assert(type == GLctx.UNSIGNED_SHORT)`),
// so a 32-bit index type aborts on the first real draw. SHADER_MAX_VERTEXES stays
// below 65536, so this is lossless — which is why the Xbox build could do it too.
