...
//MCG - Begin
	...
	if(VectorLength(diff) < Q_flrandom() * 32)
	{
		return qtrue;
	}
//MCG - End
...
qboolean NPC_CheckDefend (float scale)
{
	...
	if((float)(NPCInfo->stats.evasion) > Q_flrandom() * 4 * scale)
		return qtrue;
		...
		if( attack_ok )
		{
			...
			{//We're not going to hit him directly, try a suppressing fire
				...
				else
				{
					...
					if(aim_off > Q_flrandom() * max_aim_off)//FIXME: use aim value to allow poor aim?
					{
						attack_scale *= 0.75;
						//see if where we're going to shoot is too far from his head
						VectorSubtract(hitspot, enemy_org, diff);
						aim_off = VectorLength(diff);
						if(aim_off > Q_flrandom() * max_aim_off)
						{
							attack_ok = qfalse;
						}
					}
