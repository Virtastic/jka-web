...
typedef struct {
	...
	qboolean	g2_SetSkin(CGhoul2Info *ghlInfo, qhandle_t customSkin, qhandle_t renderSkin /*idTech3-web:=0*/)
	{
	...
	qboolean	g2_SetBoneAnim(CGhoul2Info *ghlInfo, const char *boneName, const int startFrame, const int endFrame,
					  const int flags, const float animSpeed, const int currentTime, const float setFrame /*idTech3-web:=-1*/, const int blendTime /*idTech3-web:=-1*/)
	{
...
#ifdef _IMMERSION
	ffHandle_t	(*registerForce)(const char *name, int channel /*idTech3-web:=FF_CHANNEL_MENU*/);
