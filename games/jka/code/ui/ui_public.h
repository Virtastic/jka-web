...
typedef struct {
	...
	int			R_Font_StrLenPixels(const char *text, const int setIndex, const float scale /*idTech3-web:=1.0f*/)
	{
		return RE_Font_StrLenPixels(text, setIndex, scale);
	}
	int			R_Font_HeightPixels(const int setIndex, const float scale /*idTech3-web:=1.0f*/)
	{
		return RE_Font_HeightPixels(setIndex, scale);
	}
	void		R_Font_DrawString(int ox, int oy, const char *text, const float *rgba, const int setIndex, int iMaxPixelWidth, const float scale /*idTech3-web:=1.0f*/)
	{
	...
	void		(*R_SetColor)( const float *rgba );	// NULL /*idTech3-web:=1*/,1,1,1
