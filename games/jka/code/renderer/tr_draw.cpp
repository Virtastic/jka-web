...
#include "tr_local.h"
// idTech3-web: GL_CLAMP (0x2900) is invalid in WebGL/GLES2. The direct cinematic/2D-blit
// wrap calls below need GL_CLAMP_TO_EDGE (image-creation paths already map it in R_CreateImage).
#ifdef __EMSCRIPTEN__
#define IDT3_CLAMP GL_CLAMP_TO_EDGE
#else
#define IDT3_CLAMP GL_CLAMP
#endif
...
//===========
	...
	if ( cols != tr.scratchImage[iClient]->width || rows != tr.scratchImage[iClient]->height ) 
	{
...
#if defined(__EMSCRIPTEN__)   // WebGL1 requires internalformat == format; GL_RGB8 != GL_RGBA -> white cinematic
		...
		qglTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, IDT3_CLAMP );
		qglTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, IDT3_CLAMP );
...
#elif defined(__EMSCRIPTEN__)   // WebGL1 requires internalformat == format; GL_RGB8 != GL_RGBA -> white cinematic
		qglTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA, cols, rows, 0, GL_RGBA, GL_UNSIGNED_BYTE, data );
#else
		qglTexImage2D( GL_TEXTURE_2D, 0, GL_RGB8, cols, rows, 0, GL_RGBA, GL_UNSIGNED_BYTE, data );
#endif
