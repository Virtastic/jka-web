...
int WP_FindClosestBodyPart(gentity_t *ent, gentity_t *other, vec3_t point, vec3_t out, int c = 0)
{
	...
	if (ent->client)
	{
		...
		if (where < 2 && c == 0) 
		{
			if (Q_flrandom() < .75f)		// 25% chance to actualy hit the head or eye
				where = 7;
		}
	...
	if (c == 0)
	{
		...
		float r = Q_flrandom() * 8.0f - 4.0f;
		float r2 = Q_flrandom() * 8.0f - 4.0f;
		float r3 = Q_flrandom() * 10.0f - 5.0f;
...
//------------------------------------------------------------------------------
{
	gentity_t	*missile = CreateMissile( start, fwd, 950 + Q_flrandom() * 700, 1500 + Q_flrandom() * 2000, self, qtrue );
...
//---------------------------------------------------------
{
	...
	for ( int i = 0; i < 2; i++ )
	{
		...
		dir[PITCH] -= Q_flrandom() * 4 + 8; // make it fly upwards
		dir[YAW] += crandom() * 2;
...
//---------------------------------------------------------
{
	...
	if ( alt_fire )
	{
		...
		if ( (lockEntNum > 0||ent->NPC&&lockEntNum>=0) && lockEntNum < ENTITYNUM_WORLD && lockTime > 0 )
		{
			...
			if ( dif == 8 || Q_flrandom() * dif > 2 || Q_flrandom() > 0.97f )
			{
		...
		if ( ent->client->ps.eFlags & EF_PLANTED_CHARGE )
		{
			...
			while (( found = G_Find( found, FOFS( classname ), "detpack" )) != NULL )
			{
				if ( found->activator == ent )
				{
					...
					found->nextthink = level.time + 100 + Q_flrandom() * 100;
...
void misc_weapon_shooter_fire( gentity_t *self )
{
	...
	{//repeat
		...
		if (self->random)
		{
			self->nextthink = level.time + self->wait + (int)(Q_flrandom()*self->random);
		}
		else
		{
