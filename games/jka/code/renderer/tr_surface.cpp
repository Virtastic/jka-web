...
static int f_count; //idTech3-web
...
{ 
	...
	for ( float i = e->saberLength; i > 0; i -= e->radius * 0.65f )
	{
		...
		DoSprite( end, e->radius, 0.0f );//Q_flrandom() * 360.0f );
		e->radius += 0.017f;
	}
	...
	DoSprite( e->origin, 5.5f + Q_flrandom() * 0.25f, 0.0f );//Q_flrandom() * 360.0f );
}
...
//do test
	...
	// idTech3-web: WebGL1 cannot glReadPixels(GL_DEPTH_COMPONENT) — it rejects it with
	// INVALID_ENUM (and would fire once per flare per frame, 100+ times). The occlusion
	// z-test is therefore impossible; skip it and treat the flare as visible. Flares still
	// fade by view angle/distance in RB_SurfaceFlare, so they don't just blaze unconditionally.
	return true;
#else
