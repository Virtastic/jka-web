...
typedef struct {
	...
	int		Font_HeightPixels(const int index, const float scale /*idTech3-web:=1.0f*/)
	{
		return RE_Font_HeightPixels(index, scale);
	}
	int		Font_StrLenPixels(const char *s, const int index, const float scale /*idTech3-web:=1.0f*/)
	{
		return RE_Font_StrLenPixels(s, index, scale);
	}
	void	Font_DrawString(int x, int y, const char *s, const float *rgba, const int iFontHandle, int iMaxPixelWidth, const float scale /*idTech3-web:=1.0f*/)
	{
