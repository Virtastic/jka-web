...
//------------------------------------------------------------------------------------------------------------
{
	...
	if ( mod == MOD_DEMP2 || mod == MOD_DEMP2_ALT )
	{
		...
		self->attackDebounceTime = level.time + 800 + Q_flrandom() * 500;
...
//-----------------------------------------------------
{
	...
	else
	{
		...
		self->bounceCount = level.time + 2000 + Q_flrandom() * 150;
	}
		...
		if ( !tr.allsolid && !tr.startsolid && ( tr.fraction == 1.0 || tr.entityNum == target->s.number ))
		{
			...
			if ( enemyDist < bestDist )// all things equal, keep current
			{
				if ( self->attackDebounceTime + 2000 < level.time )
				{
					...
					// Wind up turrets for a bit
					self->attackDebounceTime = level.time + 900 + Q_flrandom() * 200;
				}
...
//---------------------------------
{
	...
	if ( keep )
	{
		ent->bounceCount = level.time + 500 + Q_flrandom() * 150;
	}
...
//---------------------------------
{
	...
	if ( ent->enemy && ent->attackDebounceTime < level.time && Q_flrandom() > 0.3f )
	{
		ent->count--;
