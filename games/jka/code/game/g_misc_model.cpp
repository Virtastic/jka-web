...
void spawn_rack_goods( gentity_t *ent )
{
	...
	if ( ent->spawnflags & RACK_WEAPONS )
	{
		...
		if ( it )
		{
			...
			pos = ( Q_flrandom() > .5 ) ? -1 : 1;

			GunRackAddItem( it, ent->s.origin, ent->s.angles, crandom() * 2, ( Q_flrandom() * 6 + 4 ) * pos, v_off );
		}
	}
	...
	if (( ent->spawnflags & RACK_HEALTH ) && health )
	{
		...
		GunRackAddItem( health, ent->s.origin, ent->s.angles, crandom() * 0.5f, ( Q_flrandom() * 4 + 4 ) * pos, 24 );
	}
