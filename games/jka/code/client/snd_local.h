...
// It only runs under the OpenAL path (s_UseOpenAL, default 0), so provide a minimal shim so
// the EAX-manager code in snd_dma.cpp compiles; none of it executes with the software mixer.
typedef long HRESULT;
typedef void* HINSTANCE;
#define EM_OK 0
#define EMFLAG_IDDEFAULT (-1)
typedef struct _EMPOINT { float fX, fY, fZ; } EMPOINT;
typedef EMPOINT *LPEMPOINT;
#define EMFLAG_LOCKPOSITION 1
#define EMFLAG_LOADFROMMEMORY 2
struct IEaxManager {   // variadic methods: dead code (s_UseOpenAL 0), just needs to compile
    long GetEnvironmentAttributes(...){return -1;}
    long GetEnvironmentName(...){return -1;}
    long GetListenerDynamicAttributes(...){return -1;}
    long GetSourceDynamicAttributes(...){return -1;}
    long GetSourceID(...){return -1;}
    long GetSourceNumInstances(...){return -1;}
    long GetSourceInstancePos(...){return -1;}
    long LoadDataSet(...){return -1;}
    long FreeDataSet(...){return -1;}
...
static inline void* GetProcAddress(HINSTANCE, const char*){return (void*)0;}
static inline int FreeLibrary(HINSTANCE){return 0;}
// Win32 COM/EAX helpers the (dead) EAX 4.0 path expects. GUID comes from eax\eax.h above.
