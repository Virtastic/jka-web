...
#include "b_local.h"
#include "g_functions.h"	// idTech3-web: NPC_Pain default lives here
#include "g_nav.h"
...
void Seeker_Strafe( void )
{
	...
	if ( Q_flrandom() > 0.7f || !NPC->enemy || !NPC->enemy->client )
	{
		...
		if ( tr.fraction > 0.9f )
		{
			...
			NPCInfo->standTime = level.time + 1000 + Q_flrandom() * 500;
		}
	}
	else
	{
		...
		if ( tr.fraction > 0.9f )
		{
			...
			NPCInfo->standTime = level.time + 2500 + Q_flrandom() * 500;
		}
	}
}
...
void Seeker_FindEnemy( void )
{
	...
	if ( best )
	{
		...
		NPC->random = Q_flrandom() * 6.3f; // roughly 2pi
...
void Seeker_FollowPlayer( void )
{
	...
	else
	{
		if ( NPC->client->NPC_class != CLASS_BOBAFETT )
		{
			...
			{
				TIMER_Set( NPC, "seekerhiss", 1000 + Q_flrandom() * 1000 );
				G_Sound( NPC, G_SoundIndex( "sound/chars/seeker/misc/hiss" ));
			}
		}
