...
void G_AlertTeam( gentity_t *victim, gentity_t *attacker, float radius, float soundDist )
{
	...
	int i; // idTech3-web: hoisted (MSVC for-scope leak)
	for ( i = 0; i < 3; i++ )
	{
	...
	if ( client )
	{
		...
		if (mod==MOD_SLIME || mod==MOD_LAVA)
		{
			...
			if (mod==MOD_SLIME)
			{
				...
				{
 					testDirection[0] = (Q_flrandom() * 0.5f) - 0.25f;
					testDirection[1] = (Q_flrandom() * 0.5f) - 0.25f;
					testDirection[2] = 1.0f;
					VectorMA(targ->currentOrigin, 60.0f, testDirection, testStartPos);
					VectorCopy(targ->currentOrigin, testEndPos);
					testEndPos[0] += (Q_flrandom() * 8.0f) - 4.0f;
					testEndPos[1] += (Q_flrandom() * 8.0f) - 4.0f;
					testEndPos[2] += (Q_flrandom() * 8.0f);
...
//					CG_DrawEdge(testStartPos,	testEndPos,	EDGE_IMPACT_POSSIBLE);
					...
					TIMER_Set(targ, "AcidPainDebounce", 200 + (10000.0f * Q_flrandom() * chanceOfFizz));
					hitLoc = HL_CHEST;
				}
			}
		}
