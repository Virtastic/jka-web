...
{ 
	...
	// idt3-web: RoQ pixels upload as GL_RGBA; this packs A=0. Desktop ignores alpha on the
	// cinematic draw, but WebGL's LEGACY_GL_EMULATION respects it, so A=0 -> transparent
	// (black). Force A=0xff. (See the matching note in the JK2 tree.)
	return LittleLong((r)+(g<<8)+(b<<16)+(0xff<<24));
#else
...
e_status CIN_RunCinematic (int handle)
{
	...
	if (cinTable[currentHandle].shader && (abs((int)(thisTime - cinTable[currentHandle].lastTime)))>100) {   // idt3-web: disambiguate abs() overload under libc++
